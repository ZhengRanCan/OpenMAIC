...
export class FormalFusionError extends Error {
  constructor(readonly code: 'FUSION_SESSION_UNAVAILABLE' | 'FUSION_SESSION_MISMATCH' | 'FUSION_CONTEXT_INVALID' | 'FUSION_SESSION_ALREADY_GENERATED') {
...
export function formalFusionErrorResponse(error: FormalFusionError): NextResponse {
  ...
  return NextResponse.json(
    { success: false, errorCode: error.code, error: 'Fusion session cannot generate this classroom. Restart from Launch.' },
    { status },
  );
}
...
function createContext(record: FusionSessionRecord, requirement: unknown): FrozenTeachingContext {
  try { return createFrozenTeachingContext(record, requirement); }
  catch { throw new FormalFusionError('FUSION_CONTEXT_INVALID'); }
}

async function recoverFormalSession(request: NextRequest, lessonSessionId: unknown): Promise<FusionSessionRecord> {
  if (typeof lessonSessionId !== 'string' || !lessonSessionId) throw new FormalFusionError('FUSION_SESSION_UNAVAILABLE');
  if (!isProductionFusion()) throw new FormalFusionError('FUSION_SESSION_UNAVAILABLE');
  const record = await (await ensureFusionServices()).sessions.recover(
    request.cookies.get(COOKIE)?.value ?? '',
  );
  if (!record) throw new FormalFusionError('FUSION_SESSION_UNAVAILABLE');
  if (record.lessonSessionId !== lessonSessionId) throw new FormalFusionError('FUSION_SESSION_MISMATCH');
  return record;
}
...
export async function resolveFormalFusion(
  ...
  if (lessonSessionId === undefined || lessonSessionId === null || lessonSessionId === '') return { kind: 'none' };
...
export async function freezeFormalFusionForOutline(
  ...
  const updated = await sessions.compareAndSet(record.lessonSessionId, record.revision, (current) => ({
    ...current,
